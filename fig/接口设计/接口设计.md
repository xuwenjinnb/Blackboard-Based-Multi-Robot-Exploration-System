...
Redis 相关代码统一放在 `backend/app/redis/` 包中。该包按照配置层、客户端工具层和仓储层组织：`config.py` 负责 Redis 地址、命名空间、连接池和超时重试参数；`client.py` 负责连接池创建、连接健康检查和短暂断线重试；`blackboard.py`、`auth_store.py`、`replay_store.py` 分别负责仿真黑板、用户会话和回放数据的保存。
...
| 仿真黑板仓储 | `app/redis/blackboard.py` | `RedisBlackboard` | 保存地图、车辆、frontier、任务、导航请求、导航计划、心跳、事件和系统状态 |
