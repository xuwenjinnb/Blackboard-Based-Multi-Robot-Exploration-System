...
class RedisBlackboard(Blackboard):
    ...
    def _load_state(self) -> None:
        ...
        self.height = int(meta.get("height", self.height))
        self.chunk_size = int(meta.get("chunkSize", self.chunk_size))
        self._map_generation = int(meta.get("generation", meta.get("updatedAt", now_ms())))
        chunks = self._load_map_chunks()
        if chunks:
            cells = self._cells_from_chunks(chunks)
        else:
            cells = self._load_legacy_map_cells()
            if not cells:
        ...
        self.map = self._build_map(
            ...
            map_id=meta.get("mapId", "demo-map"),
        )
        self.map["chunks"] = chunks
        self.width = self.map["width"]
        self.height = self.map["height"]
        obstacle_key = self.key("true_obstacles")
        has_obstacle_layout = bool(
            self.redis.exists(obstacle_key)
            or self.redis.exists(self.key("true_obstacles:initialized"))
        )
        if has_obstacle_layout:
            self.true_obstacles = {
                tuple(map(int, value.split(":")))
                for value in self.redis.smembers(obstacle_key)
            }
        else:
            self.true_obstacles = self._build_truth_obstacles()
        ...
        for attribute, redis_name in self.COLLECTIONS.items():
            collection = self._load_hash(redis_name)
            if attribute == "frontiers":
                collection = {
                    field: self._sanitize_frontier(frontier)
                    for field, frontier in collection.items()
                }
            setattr(self, attribute, collection)

        self.events = [
            self._decode(fields.get("data"), {})
            for _, fields in self.redis.xrange(self.key("events"), min="-", max="+")
        ]
        ...
        self._replace_events = False
        self._system_status = self.redis.hget(self.key("system"), "status") or "STOPPED"
        self._counters = {
            name: int(value)
            for name, value in self.redis.hgetall(self.key("counters")).items()
        }
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        with self.redis.pipeline(transaction=True) as pipe:
            ...
            pipe.xrange(self.key("events"), min="-", max="+")
        ...
        if not cells:
            ...
            if not cells:
                cells = self._build_empty_cells(int(meta.get("updatedAt", now_ms())))
            chunks = self._build_chunks_from_cells(cells) if cells else []
        ...
        return {
            "map": {
                ...
                "chunkSize": int(meta.get("chunkSize", self.chunk_size)),
                "version": int(meta.get("version", 1)),
                "generation": int(meta.get("generation", meta.get("updatedAt", 0))),
                "cells": cells,
                "chunks": chunks,
